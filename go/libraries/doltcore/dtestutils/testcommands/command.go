...
func (a StageAll) Exec(t *testing.T, dEnv *env.DoltEnv) error {
	return actions.StageAllTables(context.Background(), dEnv.DoltDB, dEnv.RepoStateReader(), dEnv.GetRepoStateWriter())
}
...
func (c CommitStaged) Exec(t *testing.T, dEnv *env.DoltEnv) error {
	...
	_, err = actions.CommitStaged(context.Background(), dEnv.DoltDB, dEnv.RepoStateReader(), dEnv.GetRepoStateWriter(), actions.CommitStagedProps{
		Message:          c.Message,
		Date:             time.Now(),
		AllowEmpty:       false,
...
func (c CommitAll) Exec(t *testing.T, dEnv *env.DoltEnv) error {
	err := actions.StageAllTables(context.Background(), dEnv.DoltDB, dEnv.RepoStateReader(), dEnv.GetRepoStateWriter())
...
func (q Query) Exec(t *testing.T, dEnv *env.DoltEnv) error {
	...
	require.NoError(t, err)
	sqlDb := dsqle.NewDatabase("dolt", dEnv.DoltDB, dEnv.RepoStateReader(), dEnv.GetRepoStateWriter())
